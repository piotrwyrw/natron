...
int _chk_env(struct CompilerEnv *env) {
    if (!env->src)
        return EXIT_FAILURE;
...
void _gen_preamble(struct CompilerEnv *env) {
...
int _compile_next(struct CompilerEnv *env) {
...
void _loop_rel(struct CompilerEnv *env) {
...
unsigned int _loop_add(struct CompilerEnv *env) {
...
unsigned int _loop_del(struct CompilerEnv *env) {
    if (env->_loop_index == 0)
